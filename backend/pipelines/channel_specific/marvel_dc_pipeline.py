...
class MarvelDCChannelPipeline(BasePipeline):
    ...
    def __init__(self, output_path: Optional[str] = None, base_model: str = "stable_diffusion_1_5"):
        super().__init__("marvel_dc", output_path, base_model)
    ...
    def _optimize_video_prompt(self, prompt: str, channel_type: str = "marvel_dc", model_name: str = None) -> str:
        ...
        if model_name and model_name in BASE_MODEL_PROMPT_TEMPLATES:
            ...
            if "structure" in template and model_name in ["anything_xl", "aam_xl_animemix"]:
                ...
                year = "newest"
                optimized_prompt = f"{quality}, {year}, comic book style, superhero art, {prompt}"
            else:
                optimized_prompt = f"{template['prefix']}, comic book style, superhero art, {prompt}"
